...
//            std::string fileName = this->exec(inputPath.c_str());
            ...
            if (roadId.find(":J") == 0) {
                ...
                if (savedEdge.compare(str) != 0) {
                    savedEdge = str;
                    std::string fileName = Utility::cmdFindFile(std::to_string(myId));
                    if(fileName.length() == 0) { // write agv's data if not already exist
                        std::ofstream(std::string("agv_info/") + srcJuncSFM + "_" + destJuncSFM +
                                                + "_" + str + "_" + std::to_string(myId));
                    } else { // update agv's data
                        std::string cmdRenameFile = "mv agv_info/" + fileName + " agv_info/" + srcJuncSFM + "_" + destJuncSFM +
                                                + "_" + str + "_" + std::to_string(myId);
...
//               }
                ...
                if (this->stuckAtJunc[laneID] + waitingTime
                        < simTime().dbl()) {
                    this->runAfterStuck();
                    if (isHalting) {
                        isHalting = false;
...
void AGVControlApp::setAgentsFlow(SocialForce *socialForce, Agent *agent, float desiredSpeed,
        int caseJump) {
...
bool AGVControlApp::updateNoGraphics() {
    ...
    for (Agent *agent : agents) {
        ...
        if (agent->getVelocity().length() < Constant::LOWER_SPEED_LIMIT + 0.2
                && agent->getMinDistanceToWalls(socialForce->getWalls(), src,
                        agent->getRadius()) < 0.2
    ...
    for (SFMAGV *agv : agvs) {
        // AGV processing
        if (agv->getCollisionStartTime() == 0
                && agv->getVelocity().length() < speedConsiderAsStop
                && agv->getIsMoving()) {
        ...
        if (agv->getCollisionStartTime() != 0
            ...
            agv->setTotalStopTime(
                    agv->getTotalStopTime() + currTime
                            - agv->getCollisionStartTime());
...
std::string AGVControlApp::checkForPausing() {
    ...
    if (content.find(this->station->getName() + "_") != std::string::npos
            && this->station->getName().length() > 0 && //pausingTime == DBL_MAX
...
void AGVControlApp::readSingleMessage(jute::jValue v) {
    ...
    if (prevRoute.compare(newRoute) != 0) {
        if (newRoute.compare("0") == 0) {
            ...
            if (newRoute.compare(Constant::CARRY_ON) == 0) {
                if (prevRoute.length() == 0) {
                    ...
                    if (sp == 0
                            && pausingTime + Constant::PAUSING_TIME
                                    < simTime().dbl()) {
                        //if waiting to long
                        this->runAfterStuck();
                        return;
        ...
        if (sp == 0 && laneID[0] == ':') {
            ...
            if (this->stuckAtJunc[laneID] + Constant::PAUSING_TIME
                    < simTime().dbl()) {
                this->runAfterStuck();
            }
        }
